...
class API {
    ...
    async getAccessToken() {
        ...
        const url =
            this.prefix +
            'component/api_authorizer_token?component_access_token=' +
    ...
    async getComponentAccessToken() {
        ...
        // 过期时间，因网络延迟等，将实际过期时间提前10秒，以防止临界点
        const expireTime = new Date().getTime() + (data.expires_in - 10) * 1000;
        const componentToken = new ComponentAccessToken(data.component_access_token, expireTime);
    ...
    async ensureAccessToken() {
        ...
        if (
            token &&
            (accessToken = new AccessToken(token.accessToken, token.expireTime)).isValid()
        ) {
            return accessToken;
        }
